...
def get_args():
    ...
    parser.add_argument('--runs', type=int, default=10, help = 'number of executions to compute the average metrics')
...
def get_loaders(name, datahub_path, device, bsz, shuffle):
    ...
    if name == 'movielens1M':
        ...
        dataloaders = CIKMColdStartDataLoader(name, path, device, bsz=bsz, shuffle=shuffle)
    else:
...
def diff(model,
    ...
    warm_model = DIFF(model, 
                    ...
                    train_loader=train_base,
                      ...
                      noise_min=noise_min,
                      noise_max=noise_max,
                      eta=eta,timesteps=timesteps
                      ).to(device)
    warm_model.init_diff()
    def warm_up(dataloader, epochs, iters, logger=False):
        warm_model.train()
        criterion = torch.nn.BCELoss()
        warm_model.optimize_diff()
        optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, warm_model.parameters()), \
                                                lr=lr, weight_decay=weight_decay)
        batch_num = len(dataloader)
        # train warm-up model
        for e in range(epochs):
            for i, (features, label) in enumerate(dataloader):
                ...
                for _ in range(iters):
                    ...
                    loss = main_loss + recon_term + reg_term * diff_coef
                    warm_model.zero_grad()
                    loss.backward()
                    optimizer.step()
                    a, b, c, d = a + loss.item(), b + main_loss.item(), c + recon_term.item(), d + reg_term.item()
...
if __name__ == '__main__':
    ...
    print("var(avg_auc_list): ", np.var(np.stack(avg_auc_list), axis=0))
    print("var(avg_f1_list): ",  np.var(np.stack(avg_f1_list), axis=0))

    print("sqrt(var(avg_auc_list)): ", np.sqrt(np.var(np.stack(avg_auc_list), axis=0)))
    print("sqrt(var(avg_f1_list)): ",  np.sqrt(np.var(np.stack(avg_f1_list), axis=0)))
