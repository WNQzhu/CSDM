...
class CIKMbaseDataset(Dataset):
    ...
    def __init__(self, dataset_name, df, description, device):
        ...
        self.features = [name for name in df.columns if name != 'action']
        self.label = 'action'
...
class CIKMColdStartDataLoader(object):
    """
    ...
    def __init__(self, dataset_name, dataset_path, device, bsz=32, shuffle=True):
        ...
        for key, df in data.items():
            if key == 'description':
                continue
            self.dataloaders[key] = DataLoader(CIKMbaseDataset(dataset_name, df, self.description, device), batch_size=bsz, shuffle=shuffle)
        self.keys = list(self.dataloaders.keys())
        #self.item_features = ['item_id', 'cate_id', 'customer', 'campaign_id']
        #self.user_features = ['occupation', 'new_user_class_level']
        #self.item_features = ['item_id', 'cate_id', 'customer', 'campaign_id', 'count', 'brand', 'price']        
        self.item_features = ['item_id', 'count']
        self.user_features = ['gender', 'purchasing_power']
